...
VALUES
('nguyenO', 'Nguyễn Văn O', 'Nam', 'nguyenO@gmail.com', '123', '1996-01-01', 'Hà Nội');

DELETE FROM users
WHERE username = 'nguyenO';


-- Bài 2:
CREATE VIEW view_user_post AS
SELECT users.user_id, COUNT(posts.post_id) AS total_post
FROM users
LEFT JOIN posts ON users.user_id = posts.user_id
GROUP BY users.user_id;

SELECT users.full_name, view_user_post.total_post
FROM users
JOIN view_user_post
ON users.user_id = view_user_post.user_id;
